...
from collators.qwen_vision_process1 import process_vision_info
...
def construct_messages_mr_fps(video_path, feature_path, fps, retrieval_segment, retrieval_mode, clip_length, query):
        ...
        message = [
            {
                "role": "user",
                "content": [
                    {"type": "video", "video": f"{video_path}", "fps": fps, "video_start": retrieval_segment[0], "video_end": retrieval_segment[1], "query":query},
...
def run_inference(model, processor, data, args, device):
    ...
    #print("temporal_windows:", temporal_windows)
    ...
    #     video_path, feature_path, args.fps, retrieval_segment, retrieval_mode, args.clip_length, query
    # )
    # messages = [message]
    ...
    for query, temporal_window in zip(querys, temporal_windows):
        ...
        if feature_inputs is None:
            ...
            all_timestamps_num = all_timestamps_combine
        all_timestamps = [[f"timestamp: {all_t} seconds; feature: " for all_t in sublist] for sublist in all_timestamps_num]
        frame_timestamps = all_timestamps_num[0] if len(all_timestamps_num) > 0 else []

        print("input_timestamps:", all_timestamps_num)
        ...
        print("MODEL_OUTPUT:", output_text)
...
def inference_worker(rank, world_size, args):
    ...
    # Configure device_map and optional 4bit quantization
    use_shard = getattr(args, "shard_model", False)
    device_map_value = "auto" if use_shard else {"": device}
    max_memory = None
    if use_shard:
        
        total_gpus = torch.cuda.device_count()
        max_memory = {i: str(torch.cuda.get_device_properties(i).total_memory // (1024**3) - 2) + "GiB" for i in range(total_gpus)}
        offload_folder = os.path.join(args.output_dir, "offload_cache")
        os.makedirs(offload_folder, exist_ok=True)
    quantization_config = None
    if getattr(args, "load_in_4bit", False):
        try:
            from bitsandbytes import __version__ as _
            ...
            raise RuntimeError(
                f"启用 --load_in_4bit 需要安装 bitsandbytes 与 transformers>=4.38；请先安装。原始错误: {e}"
            )

    if args.model_finetune_path:
        from peft import PeftModel
        base_model = Qwen2VLMRForConditionalGeneration.from_pretrained(
        ...
        model = PeftModel.from_pretrained(base_model, args.model_finetune_path)
        
        if not getattr(args, "load_in_4bit", False):
...
def main():
    ...
    parser.add_argument('--model_local_path', type=str, default='UniTime/checkpoints/model_local_path', help='Path to the local model')
    parser.add_argument('--output_dir', type=str, default='./results/test')
    parser.add_argument('--model_finetune_path', type=str, default='UniTime/checkpoints/model_finetune_path', help='Path to the finetune model')
    parser.add_argument('--video_root', type=str, default='/media/vlilab/DATA/MEMBER/QIANG/data/Charades_v1/videos' ,help='Path to the video root')
    parser.add_argument('--data_path', type=str, default= 'UniTime/data/test.json' ,help='Path to the feature folder')
    ...
    # 新增：控制显存与多卡使用
    # parser.add_argument('--shard_model', action='store_true', help='单进程使用 device_map=auto 在多卡间分片模型')
    # parser.add_argument('--load_in_4bit', action='store_true', help='使用bitsandbytes 4bit量化加载模型以降低显存')
    parser.add_argument('--shard_model', default=True, help='单进程使用 device_map=auto 在多卡间分片模型')
    parser.add_argument('--load_in_4bit', action='store_true', help='使用bitsandbytes 4bit量化加载模型以降低显存')
    parser.add_argument('--num_processes', type=int, default=None, help='多进程推理的进程数（默认等于可见GPU数）')
    ...
    if getattr(args, 'shard_model', False):
        inference_worker(0, 1, args)
    else:
        if world_size > 1:
            nprocs = args.num_processes if args.num_processes is not None else world_size
            ...
            merge_results(args.output_dir, nprocs)
        else:
            # Single GPU case
            inference_worker(0, 1, args)
